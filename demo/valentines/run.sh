...
SCENARIO_PATH="$ROOT_DIR/demo/valentines/scenario.yaml"
LOG_DIR="$ROOT_DIR/demo/valentines/logs"
mkdir -p "$LOG_DIR"
STAMP="$(date +%Y%m%d_%H%M%S)"
LLM_LOG="$LOG_DIR/valentines_llm_${STAMP}.log"
DEMO_TICKS=15

if [ "$#" -gt 0 ]; then
  echo "This script uses fixed demo settings and does not accept arguments."
  echo "Run: bash demo/valentines/run.sh"
  exit 2
fi
...
uv run python demo/judge_summary.py --scenario valentines --llm-log "$LLM_LOG"
